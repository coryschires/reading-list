...
Had never thought about this being a language feature:
    ...
    contains the method’s name, along with any parameters the method 
    may need. 33
...
An excellent, succinct explanation of how <tt>yield</tt> works:
    ...
    You can think of yield as being something like a method call that 
    invokes the block associated with the call to the method containing 
    the yield. 44
    
What does this mean?

    You may be better off thinking of the block and the method as 
    coroutines, which transfer control back and forth between 
...
Didn't know about ARGF:
    ...
    ...the variable ARGF is a special kind of I/O object that acts like 
    all the contents of all the files whose names are passed on the command 
    line (or standard input if you don’t pass any filenames). 46
...
Never thought of restricted methods this way. Always thought of them simply as internal methods (i.e. methods intended to be called only by other methods).
    ...
    A good rule of thumb is never to expose methods that could leave an object 
    in an invalid state.
